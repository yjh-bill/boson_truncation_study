...
ind_minus = -1
for (nc, corr_indices) in enumerate(corr_watch_ls)
    if site_minusR2 == corr_indices
        global ind_minus = nc
    end
end
...
function estimate_R2_ls(confs, p_x, p_y; lambda_list=eig_vals, U_mat=U)
    # confs shape (N_cf, N_t, N_s, N_s)
    confs_newshape = (size(confs)[1:end-2]..., size(confs)[end-1] * size(confs)[end]) # (N_cf, N_t, N_s * N_s)
    confs_view = reshape(confs, confs_newshape)

    confs_C = similar(confs_view)
    @inbounds for i in axes(confs_C, 1)
        confs_C[i, :, :] = confs_view[i, :, :] * U_mat
    end
    delta_C = confs_C .- circshift(confs_C, (0, 1, 0)) # (N_cf, N_t, N_s^2)

    p_i = plaquette_coord_to_i(p_x, p_y, Ns)
    # S2_terms = meanr(Hamiltonian_kin_terms(confs; lambda_list, U_mat), dims=( 2)) .* 2 ./ g^2 ./ reshape(lambda_list, (1, Ns^2)) # (N_cf, Ns)

    term_1 =  sum(1. ./ lambda_list .* U_mat[p_i, :].^2 ) / a_0 / g^2  # scalar result

    n_cf = size(confs, 1)
    n_t  = size(confs, 2)
    term_2_ls = zeros(n_cf, n_t)

    @inbounds for i in 1:Ns^2, j in 1:Ns^2
        for i_t in axes(delta_C, 2)
            for i_cf in axes(delta_C, 1)
                term_2_ls[i_cf, i_t] += (-1) * U_mat[p_i,i] * U_mat[p_i,j] * delta_C[i_cf, i_t, i] * delta_C[i_cf, i_t, j] / (lambda_list[i]*lambda_list[j])
            end
        end
    end

    term_2 = meanr(term_2_ls, dims=2) ./ (a_0 * g^2)^2 # (N_cf)
    
    return term_1 .+ term_2 # (N_cf)
...
#     return mean(estimate_R2_ls(confs, p_x, p_y; lambda_list, U_mat))
# end

function estimate_R2_mean(confs; lambda_list=eig_vals, U_mat=U)
    n_cf = size(confs, 1)
    n_t  = size(confs, 2)
    ...
    tmp1_arr   = zeros(Ns^2, Ns^2)
    @inbounds for i in 1:Ns^2, j in 1:Ns^2
        for i_t in axes(delta_C, 2)
            for i_cf in axes(delta_C, 1)
                tmp1_arr[i, j] += (-1) * delta_C[i_cf, i_t, i] * delta_C[i_cf, i_t, j] / (lambda_list[i]*lambda_list[j])
            end
        end
    end
    tmp1_arr ./= (n_cf * n_t)  

    R2_mean_ls = zeros(Ns, Ns)
    for p_x in 1:Ns, p_y in 1:Ns
        p_i = plaquette_coord_to_i(p_x, p_y, Ns)
        term_1 =  sum(1. ./ lambda_list .* U_mat[p_i, :].^2 ) / a_0 / g^2 
        term_2 = 0
        @inbounds for i in 1:Ns^2, j in 1:Ns^2
            term_2 += U_mat[p_i,i] * U_mat[p_i,j] * tmp1_arr[i, j]
        end
        term_2 /= (a_0 * g^2)^2
        R2_mean_ls[p_x, p_y] = term_1 + term_2
    end
...
function Metropolis_single(S_tloc! = S_lat_tloc! , get_Hamiltonian = Hamiltonian_tot ; Nt::Int = Nt, Ns::Int = Ns,
        n_batches::Int = n_batches, batch_size::Int = batch_size, lambda_list=eig_vals, U_mat=U, p::Progress, corrs_watch::Vector{Tuple{Int, Int}})
    lat = zeros(Nt, Ns, Ns)
    ...
    R2_i_list = Array{Float64}(undef, batch_size * n_batches, n_corrs)
    ...
    for n in 1:n_batches
        ...
        energies[i_s:i_e] = get_Hamiltonian(paths; lambda_list, U_mat)
        ...
        R2_mean_ls[:, :, n] = estimate_R2_mean(paths; lambda_list, U_mat)
        for (nc, corr_indices) in enumerate(corrs_watch)
            px, py = corr_indices
            corrs_vals_ls[:, n, nc] = [get_correlator(paths, t, px, py) for t in 0:Nt-1]
            R2_i_list[i_s:i_e, nc] = estimate_R2_ls(paths, px, py; lambda_list, U_mat)
        end
    end
    # if debug
    ...
    result = MCSingleSetResult(;energies, R2_i_list, B2_mean, R2_mean, accept_rate, corrs)
...
function Metropolis_parallel(S_tloc! = S_lat_tloc! , get_Hamiltonian = Hamiltonian_tot ; debug=false, Nt::Int = Nt, Ns::Int = Ns, 
    n_batches::Int = n_batches, batch_size::Int = batch_size, N_sets::Int = N_sets, lambda_list=eig_vals, U_mat=U, corrs_watch::Vector{Tuple{Int, Int}})
    ...
    corrs = Array{CorrResult}(undef, n_corrs)
    R2_i_list = Array{Float64}(undef, N_cf_tot, n_corrs)

    Threads.@threads for i in 1:N_sets
        i_start = (i-1) * N_cf_s + 1
        i_end = i_start + N_cf_s - 1

        result_i = Metropolis_single(S_tloc! , get_Hamiltonian; Nt, Ns, n_batches, batch_size, p, corrs_watch, lambda_list, U_mat)
        ...
        for (nc, corr_indices) in enumerate(corrs_watch)
            ...
            R2_i_list[i_start:i_end, nc] = result_i.R2_i_list[:, nc]
        end
    end
...
#%% 
R2_i_unmodified_mean = mean(MC_result.R2_i_list[:, ind_minus])
# R2_i_unmodified_bin_sizes, R2_i_unmodified_means, R2_i_unmodified_errors = binning_analysis(MC_result.R2_i_list[:, ind_minus])
binning_unmodified_R2_i = MCBinningResult(binning_analysis(MC_result.R2_i_list[:, ind_minus])...)
R2_i_unmodified_delta = find_err_from_binning(binning_unmodified_R2_i.errs; threshold=plateau_threshold)
...
function get_MC_results_minusR2(p_i; to_plot_energy=true, to_plot_correlator=false, subtract_ratio = 1/2, print_binning=false,  corrs_watch = corr_watch_ls)
    p_x, p_y = plaquette_i_to_coord(p_i, Ns)
    @assert (p_x, p_y) in corr_watch_ls
    ind_corr_ls = -1

    for (nc, corr_indices) in enumerate(corrs_watch)
        if (p_x, p_y) == corr_indices
            ind_corr_ls = nc
        end
    end

    
    m_quad_mod = deepcopy(m_quad)
    m_quad_mod[p_i, p_i] -= eta_max * subtract_ratio
    eta = eta_max * subtract_ratio
    eig_vals_mod, U_mod = eigen(m_quad_mod)

    println("\n\n---------------------R2--------------------")

    if minimum(eig_vals_mod) <= 0.
        throw("Matrix not positive-definite. Minimum eigenvalue is $(min(eig_vals_mod))")
    end

    # S_lat_tmp!(lat::Array{Float64, 3}, S_lat_draft::Vector{Float64}, wc::MCWorkspace) = let 

    #     S_lat_tloc!(lat, p_x, p_y, S_lat_draft, wc; subtract_ratio)
    # end

    S_lat_tmp!(lat::Array{Float64, 3}, S_lat_draft::Vector{Float64}, wc::MCWorkspace) = 
    let eig_vals_mod = eig_vals_mod, U_mod = U_mod
        S_lat_tloc!(lat, S_lat_draft, wc; lambda_list=eig_vals_mod, U_mat=U_mod)
    end

    # S_lat_tmp = lat -> S_lat_tloc(lat, lambda_list=eig_vals_mod, U_mat=U_mod)
    MC_R2_result = Metropolis_parallel(S_lat_tmp!; debug=true, corrs_watch, lambda_list=eig_vals_mod, U_mat=U_mod)
    # println("Configuration mean $(mean(confs_R2k))")
    # println("Configuration avg $(std(confs_R2k))")

    gs_minusR2_energies = MC_R2_result.energies
    gs_minusR2_energies_mean  = mean(gs_minusR2_energies)
    gs_minusR2_energies_std   = std(gs_minusR2_energies)
    bin_sizes, means, errors = binning_analysis(gs_minusR2_energies)
    gs_minusR2_energies_delta = find_err_from_binning(errors; threshold=plateau_threshold)
    println("Average energy is $gs_minusR2_energies_mean")
    println("Std of energy is $gs_minusR2_energies_std")
    println("Uncertainty of energy: $gs_minusR2_energies_delta")

    if to_plot_energy
        fig = Figure()
        ax = Axis(fig[1, 1], xlabel="configuration label", ylabel="Energy (1/a)", title="$p_x, $p_y")
        scatter!(ax, collect(1:length(gs_minusR2_energies)), gs_minusR2_energies)
        display(fig)
    end

    energy_diff = gs_unmodified_energies_mean - gs_minusR2_energies_mean
    energy_diff_delta = hypot(gs_unmodified_energies_delta, gs_minusR2_energies_delta)
    R2bound = energy_diff * 2 / (g^2 * eta)
    R2bound_sqrt = R2bound>=0 ? sqrt(R2bound) : NaN
    R2bound_with_delta   = (energy_diff + energy_diff_delta) * 2 / (g^2 * eta)
    R2bound_with_3delta = (energy_diff + 3 * energy_diff_delta) * 2 / (g^2 * eta)
    R2bound_sqrt_with_delta = R2bound_with_delta >=0 ? sqrt(R2bound_with_delta) : NaN
    R2bound_sqrt_with_3delta = R2bound_with_3delta >=0 ? sqrt(R2bound_with_3delta) : NaN


    R2_i_bin_sizes, R2_i_means, R2_i_errors = binning_analysis(MC_R2_result.R2_i_list[:, ind_corr_ls])
    binning_R2_i = MCBinningResult(R2_i_bin_sizes, R2_i_means, R2_i_errors)
    R2_i_R2_delta = find_err_from_binning(binning_R2_i.errs; threshold=plateau_threshold)

    if print_binning
        # bin_sizes, means, errors = binning_analysis(gs_minusB2_energies)
        println("Binning result for the energy")
        print_binning_results(bin_sizes, means, errors)
        println("\n\nBinning result for R^2 at the site")
        print_binning_results(R2_i_bin_sizes, R2_i_means, R2_i_errors)
    end
    ...
    println("This means <R^2> is bounded by $R2bound, whose square root is $R2bound_sqrt")
    println("Or with one delta, <R^2> is bounded by $(R2bound_with_delta), whose square root is $(R2bound_sqrt_with_delta)")
    println("Or with three deltas, <R^2> is bounded by $(R2bound_with_3delta), whose square root is $(R2bound_sqrt_with_3delta)")


    R2_ref = mean(MC_result.R2_mean)
    R2_i_ref = mean(MC_result.R2_i_list[:, ind_corr_ls])
    R2_i_R2_mean = mean(MC_R2_result.R2_i_list[:, ind_corr_ls])
    println("\nAs a reference, for the unmodified Hamiltonian <R^2> is $(R2_ref)\n\n")
    println("Also, for the unmodified vacuum, <R_i^2> is $(R2_i_ref), the sqrt of which is $(sqrt(R2_i_ref))")
    println("Also, for the modified vacuum, <R_i^2> is $(R2_i_R2_mean), the sqrt of which is $(sqrt(R2_i_R2_mean))")
    println("To be more exact, it's $(R2_i_R2_mean)±$(R2_i_R2_delta)")
    println("\n\n")

    binning_R2_energies = MCBinningResult(bin_sizes, means, errors)
    
    result_dict = Dict(
        "energy"       => gs_minusR2_energies,
        "energy_mean"  => gs_minusR2_energies_mean,
        "energy_delta" => gs_minusR2_energies_delta,
        "energy_binning" => binning_R2_energies,
        "R2bound"      => R2bound,
        "subtract_ratio" => subtract_ratio,
        "R2_i"          => MC_R2_result.R2_i_list,
        "R2_i_mean"     => R2_i_R2_mean,
        "R2_i_delta"    => R2_i_R2_delta,
        "R2_i_binning"  => binning_R2_i
    )
    ...
    if to_plot_correlator
        for corr_result in MC_R2_result.corrs
            f = plot_correlator(collect(0:Nt-1), corr_result.vals)
            px = corr_result.px
            py = corr_result.py
            save_name = joinpath(fig_path, "$(name)_R2_minus_site$(p_i)_corr_$(px)_$(py).pdf")
            save(save_name, f)
        end
    end

    
    return result_dict, MC_R2_result
end 
...
site_i = plaquette_coord_to_i(site_minusR2...)

@time result_dic_R2, MC_R2_result = get_MC_results_minusR2(site_i; to_plot_energy, to_plot_correlator, print_binning, subtract_ratio)
...
HDF5.h5open(data_name, "w") do file
    ...
    file["unmodified"]["R2_i_mean"] = R2_i_unmodified_mean
    file["unmodified"]["R2_i_delta"] = R2_i_unmodified_delta
    ...
    HDF5.create_group(file, "R2")
    file["R2"]["description"] = "Bounding all R2 at site"
    file["R2"]["p_x"] = site_minusR2[1]
    file["R2"]["p_y"] = site_minusR2[2]
    file["R2"]["energy_mean"] = result_dic_R2["energy_mean"]
    file["R2"]["energy_delta"] = result_dic_R2["energy_delta"]
    file["R2"]["R2_i_mean"] = result_dic_R2["R2_i_mean"]
    file["R2"]["R2_i_delta"] = result_dic_R2["R2_i_delta"]
    file["R2"]["subtract_ratio"] = result_dic_R2["subtract_ratio"]
    HDF5.create_group(file["R2"], "binning_energy")
    HDF5.create_group(file["R2"], "binning_R2_i")
    file["R2/binning_energy"]["bin_sizes"] = result_dic_R2["energy_binning"].bin_sizes
    file["R2/binning_energy"]["means"] = result_dic_R2["energy_binning"].means
    file["R2/binning_energy"]["errs"] = result_dic_R2["energy_binning"].errs
    file["R2/binning_R2_i"]["bin_sizes"] = result_dic_R2["R2_i_binning"].bin_sizes
    file["R2/binning_R2_i"]["means"] = result_dic_R2["R2_i_binning"].means
    file["R2/binning_R2_i"]["errs"] = result_dic_R2["R2_i_binning"].errs
    ...
    HDF5.create_group(file["R2"], "correlations")
    for corr_result in MC_R2_result.corrs
        px = corr_result.px
        py = corr_result.py
        corr_g = HDF5.create_group(file["R2/correlations"], "$(px)_$(py)")
        corr_g["px"]   = px
        corr_g["py"]   = py
        corr_g["vals"] = corr_result.vals
    end
